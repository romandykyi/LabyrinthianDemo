...
public class SvgRendererState
{
    ...
    public MazeSvgExporter GetExporter(MazeGenerator generator, bool isGenerated, bool minify = false)
    {
        ...
        MazeSvgExporterBuilder builder = MazeSvgExporterBuilder.For(maze)
        ...
        {
            builder.AddBackground(BackgroundFill);
        }

        #region Cells
        if (AllCellsGroup != null)
        {
            builder.AddAllCells(AllCellsGroup);
        }
        if (UnvisitedCellsGroup != null)
        {
            builder.AddUnvisitedCells(generator, UnvisitedCellsGroup);
        }
        if (HighlightedCellsGroup != null)
        {
            builder.AddHighlightedCells(generator, HighlightedCellsGroup);
        }
        if (SelectedCellGroup != null)
        {
            builder.AddSelectedCell(generator, SelectedCellGroup);
        }
        #endregion
        ...
        if (WallsPath != null)
        {
            ...
            {
                builder.AddWallsAsSeparatePaths(path: WallsPath, wallsWidth: WallsWidth);
            }
            else
            {
                builder.AddWallsAsSeparatePaths(path: WallsPath, wallsWidth: WallsWidth);
        ...
        if (EdgesPath != null)
        {
            builder.AddBaseGraphEdges(EdgesPath);
        }
        if (PassagesPath != null)
        {
            builder.AddPassagesGraphEdges(PassagesPath);
        }
            ...
            generator is IDirectedGraphGenerator directedGraphGenerator)
        {
            ...
            builder.AddDirectedEdges(directedEdges, DirectedEdgesGroup, path);
        }
        #endregion

        if (SolutionPath != null && isGenerated)
        {
            builder.AddSolutions(pathCreator: _ => SolutionPath);
        }

        #region Nodes
        if (NodesGroup != null)
        {
            builder.AddAllNodes(NodeShape, NodesGroup);
        }
        if (UnvisitedNodesGroup != null)
        {
            builder.AddUnvisitedNodes(generator, NodeShape, UnvisitedNodesGroup);
        }
        if (HighlightedNodesGroup != null)
        {
            builder.AddHighlightedNodes(generator, NodeShape, HighlightedNodesGroup);
        }
        if (SelectedNodeGroup != null)
        {
            builder.AddSelectedNode(generator, NodeShape, SelectedNodeGroup);
        }
        #endregion
